package org.eclipse.slm.awx.client.test.testcontainer;
...
public class AwxClientCapabilityTest {
    ...
    private static boolean containerAvailable = true;
    ...
    private static File resolveComposeFile() {
        try {
            var resource = Objects.requireNonNull(
                    AwxClientCapabilityTest.class.getClassLoader().getResource("docker-compose.yml"),
                    "docker-compose.yml not found on test classpath"
            );
            return new File(resource.toURI());
        } catch (URISyntaxException e) {
            throw new IllegalStateException("Failed to resolve docker-compose.yml path", e);
        }
    }

    static {
        awxContainer = new DockerComposeContainer(resolveComposeFile())
    ...
    static void startContainer() {
        try {
            ...
            LoggerFactory.getLogger(AwxClientCapabilityTest.class)
                    .warn("Skipping test class because AWX test container failed to start", e);
        }
    }
    ...
    public void beforeEach() {
        Assumptions.assumeTrue(containerAvailable, "AWX test container is not available");
    ...
    @Configuration
    @ComponentScan("org.eclipse.slm.awx")
